...
interface ChatContextValue {
   ...
   * and resume the agentic loop on the **same** turn. Returns `false` when
   * the active session has no live turn waiting on input, so the card can
  ...
  const submitUserReply = useCallback(
    (
      ...
      try {
        ...
        console.error("Unable to submit ask_user reply", error);
        return false;
      }
    },
    [sendThroughRunner],
